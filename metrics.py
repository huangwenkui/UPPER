...
def get_class(embeddings, test, y, logging):
    ...
    y_pre_sig_list = y_pre_sig.tolist()
    fpr, tpr, thresholds = roc_curve(y_true, y_pre_sig_list)
    youden = tpr - fpr
    cutoff = thresholds[np.argmax(youden)]
    y_pre_std = [1 if i > cutoff else 0 for i in y_pre_sig_list]
    evaluation = {}
    evaluation['auc'] = round(roc_auc_score(y_true, y_pre_sig_list), 6)
    evaluation['acc'] = round(accuracy_score(y_true, y_pre_std), 6)
    evaluation['recall'] = round(recall_score(y_true, y_pre_std), 6)
    evaluation['F1'] = round(f1_score(y_true, y_pre_std), 6)
    evaluation['average_precision'] = round(average_precision_score(y_true, y_pre_sig_list), 6)
