...
ignore_attrs = ['True Label', 'CHROM', 'Nuc-Pos', 'REF-Nuc', 'ALT-Nuc', 'Ensembl-Gene-ID', 'Ensembl-Protein-ID',
                'Ensembl-Transcript-ID', 'Uniprot-Accession','Consequence','IMPACT',"Amino_acids", "cDNA_position",'BIOTYPE','GERP_RS_rankscore','GenoCanyon_rankscore',
...
if filename == "./cancer_gnomad/03cancer_gnomAD_test_desc.csv":
    name = "cancer_gnomad"
elif filename == "./PPARG/00PPARG_desc.csv":
    name = "PPARG"
else:
    print("error")
id_data = []
...
for line in fp:
    ...
    for id_name in id_names:
        ...
        for el in id_el.split("&"):
            if (el != ".") & (el != "-") & (el != ""):
                if el=="common":
                    id_data.append([key_id, id_name, el])
                else:
                    if float(el) > 0.01:
